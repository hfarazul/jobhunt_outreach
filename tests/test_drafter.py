...
def test_draft_returns_cleaned_body_on_success(monkeypatch, db_env):
    ...
    body = (
        ...
        "I'm a senior engineer — the last 5 years I've been building production "
        "AI systems and shipping v1s under tight timelines. I saw you opened a "
        "role on the platform team and wanted to reach out directly rather than "
        "through the form. Curious what the biggest engineering challenge is for "
        "you right now, or what a strong first 90 days would look like?"
    )
    monkeypatch.setattr(
        drafter,
        "_invoke_claude",
...
@pytest.mark.parametrize("spam_text,expected_phrase", [
    ...
    ("I'd love to chat about the senior engineer role you opened this quarter.",
     "i'd love to chat"),
...
@pytest.mark.parametrize("clean_text", [
    ...
    "Bay Area for the next few weeks — worth a quick hello if you've got time. I'm a senior engineer, 5 yrs shipping prod AI. Curious what the team's building right now.",
...
@pytest.mark.parametrize("labeled_text,expected_phrase", [
    # Segment-language a candidate drafter would wrongly parrot at the reader
    ("Most hiring managers hit this exact tradeoff when scaling a team.",
     "hiring managers"),
    ("That's the kind of thing recruiters keep reaching out to me about.",
     "recruiters"),
    ("I know talent acquisition teams are slammed this time of year.",
     "talent acquisition"),
    ("Reaching out to decision makers like you about the role directly.",
     "decision makers"),
    ("I usually only hear from decision-makers once a req is open.",
     "decision-makers"),
])
...
@pytest.mark.parametrize("clean_text", [
    ...
    "Guy — the platform relaunch you shipped caught my eye. How are you thinking about the team's roadmap this quarter?",
    ...
    "The role you opened looks like a strong fit for what I've been building.",
    "You posted about scaling the team — curious what the first hire focuses on.",
    # a single word like "recruiter" in a different sense shouldn't over-match the plural label
    "I read your note on how you run technical interviews — sharp take.",
])
...
def test_draft_retries_on_audience_label(monkeypatch, db_env):
    ...
    labeled = ("Your post on the patent platform stood out. I know hiring "
               "managers are slammed this quarter. Curious what you're working on. "
               + "X" * 60)
    clean = ("Your post on the patent platform stood out. I've shipped a few "
    ...
    assert "audience" in retry_prompt or "hiring managers" in retry_prompt
