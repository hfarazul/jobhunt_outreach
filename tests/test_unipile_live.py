...
def _self_profile_url() -> str:
    # The live-account owner's own profile URL — provided via env so this repo
    # carries no identity. Skip the self-post tests when it isn't set.
    url = os.environ.get("LINKEDIN_SELF_PROFILE_URL")
    if not url:
        pytest.skip("set LINKEDIN_SELF_PROFILE_URL to run self-profile live tests")
    return url
...
def test_get_recent_posts_on_self(adapter: UnipileAdapter):
    ...
    posts = adapter.get_recent_posts(_self_profile_url(), limit=3)
    if not posts:
...
def test_react_to_own_post(adapter: UnipileAdapter):
    ...
    posts = adapter.get_recent_posts(_self_profile_url(), limit=1)
    if not posts:
