...
# describe WHO we're reaching out to, which the drafter would occasionally
# parrot back at the recipient ("reaching out as one engineering leader to
# another..."). A message that names the reader's category reads as templated
# and breaks the peer-to-peer tone the connect note depends on.
#
...
# recipient whose actual title is "Engineering Manager" referenced naturally)
...
def draft(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        # "hiring managers" internally, and the model occasionally echoes it
        # back at the recipient. That reads as marketing copy. Reject + retry.
        ...
        if label:
            ...
            retry_hint = (
                ...
                f"{label!r}. Hard rule #8: never label the recipient by their "
                f"category. Rewrite using 'you' / 'your team' or refer to the "
                f"specific situation ('the role you just opened', 'the team "
                f"you're scaling') — not the segment name."
            )
            continue
